...
const UserPolls = () => {
  ...
  useEffect(() => {
    const fetchUserAndPolls = async () => {
      ...
      try {
        ...
        const pollsResponse = await axios.get(`${API_URL}/api/users/${userId}/polls`);
        setPolls(pollsResponse.data);
      } catch (err) {
        console.error(`Error fetching polls for user ${userId}:`, err);
        setError(err.response?.data?.message || `Failed to fetch polls for user ${userId}.`);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return <div className="user-polls-container">Loading polls for {userName}...</div>;
  }
  ...
  if (polls.length === 0) {
    return <div className="user-polls-container">No polls found for {userName}.</div>;
  }
